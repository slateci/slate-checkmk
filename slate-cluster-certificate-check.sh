...
if [ $(date -d "$ENDDATE" +%s) -gt $(date -d "now + 90 days" +%s) ]; then
    echo 0 SLATE-cluster-$clustername-certificate - Certificate is valid and willl expire in more than 90 days: $IP \($ENDDATE\)
else

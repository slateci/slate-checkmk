...
if [ -n "$ENDDATE" ]; then
  if [ $(date -d "$ENDDATE" +%s) -gt $(date -d "now + 60 days" +%s) ]; then
    ...
    if [ $(date -d "$ENDDATE" +%s) -gt $(date -d "now + 15 days" +%s) ]; then
      ...
      ./mailgun.sh no-reply@slateci.io "$EMAIL, slateci-ops@googlegroups.com" "SLATE: Certificate expiration" "The certificate for cluster $clustername IP $IP needs to be renewed (expiration date $ENDDATE). For help on how to renew a Kubernetes certificate, see <a href=\"https://slateci.io/docs/resources/k8s-certificates.html\">https://slateci.io/docs/resources/k8s-certificates.html</a>"
    fi
  fi
else
